...
public class CartService {
    ...
    public List<Cart> queryCarts() {
        ...
        if (!CollectionUtils.isEmpty(unLoginCarts)){
            unLoginCarts.forEach(cart -> {
                if (loginHashOps.hasKey(cart.getSkuId().toString())){
                    String  longingJsonCart= loginHashOps.get(cart.getSkuId().toString()).toString();
                    Cart loginCart = JSON.parseObject(longingJsonCart, Cart.class);
                    Integer loginCount = loginCart.getCount();
                    loginCart.setCount(loginCount+cart.getCount());
                    loginHashOps.put(cart.getSkuId().toString(),JSON.toJSONString(loginCart));
                }else {
        ...
        List<Cart> loginCarts = null;
            ...
            loginCarts = loginCartJsonList.stream().map(cartJson -> {
                Cart cart = JSON.parseObject(cartJson.toString(), Cart.class);
                //查询当前价格
                String priceString = this.redisTemplate.opsForValue().get(PRICE_PREFIX + cart.getSkuId());
                cart.setCurrentPrice(new BigDecimal(priceString));
